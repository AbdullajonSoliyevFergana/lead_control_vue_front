          ...
          <li class="nav-item mr-auto">
            <b-link class="navbar-brand" to="/">
              <span class="brand-logo">
                <b-img
                  :src="skin == 'light' ? appLogoImageWhite : appLogoImageBlack"
                  alt="logo"
                />
              </span>
              <h2 class="brand-text">
...
export default {
  ...
  setup(props) {
    ...
    // App Name
    const { appName, appLogoImageWhite, appLogoImageBlack } = $themeConfig.app;
  ...
  methods: {
    filter() {
      ...
      if (user.role == "operator") {
        const routes = this.navMenuItems.filter((elem) => {
          if (elem.route == "leads") return elem.route;
        });
